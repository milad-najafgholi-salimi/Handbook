...
![singly linked list](img_linkedlists_singly.svg)
...
![doubly linked list](img_linkedlists_doubly.svg)
...
![singly circular linked list](img_linkedlists_circsingly.svg)

The image below is an example of a doubly circular linked list:
![doubly circular linked list](img_linkedlists_circdoubly.svg)
...
Lookup/search is always O(n). The O(1) advantage only applies to insert/delete **at the ends** (or if you already have a pointer to the node).